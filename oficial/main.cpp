...
// ===============================
//  Верхний счётчик (боты+игроки)
//   - Всегда зелёный по умолчанию
//   - Становится красным, если хотя бы один игрок найден
//   - Значение = PlayerCount + BotCount
// ===============================
static void DrawTopCenterCounter()
{
    ...
    // Всегда зелёный; красный — только при наличии игроков
...
static void RenderZKRoot() {
    ...
    float zkTargetScale = zkWasActive ? 1.2f : 1.0f; // плавное увеличение при зажатии
    zkPressScale += (zkTargetScale - zkPressScale) * ImClamp(12.0f * dt2, 0.0f, 1.0f);
    // Уменьшаем базовый размер ZK кнопки в 2 раза
    ...
    if (te>0.1f) {
        ...
        // Поворот меню на 90°: вкладки теперь сверху горизонтально, контент ниже
        ...
        if (Tab==6) {
            ...
            ImGui::Text("%s", i18n::TL("物 资").c_str());
            ImGui::Separator();
...
void 绘制_1() {
    ...
    Toggle(i18n::TL("  方框").c_str(),DrawIo[1]);
    
    Toggle(i18n::TL("  骨骼").c_str(),DrawIo[3]);
    
    Toggle(i18n::TL("  射线").c_str(),DrawIo[2]);
    
    Toggle(i18n::TL("  距离").c_str(),DrawIo[5]);
    
    Toggle(i18n::TL("  雷达").c_str(),DrawIo[7]);
    
    Toggle(i18n::TL("  背敌").c_str(),DrawIo[8]);
    
    Toggle(i18n::TL("  人机").c_str(),DrawIo[17]);
    Toggle(i18n::TL("  仅真人(自瞄)").c_str(),DrawIo[31]);
    Toggle(i18n::TL("  瞄准时不绘制").c_str(),DrawIo[46]);
    
    // Removed: back-aim WIP toggle (always on or deprecated)
    
    Toggle(i18n::TL("  忽地").c_str(),DrawIo[32]);
    
    Toggle(i18n::TL("  手持").c_str(),DrawIo[10]);
    
    Toggle(i18n::TL("  手雷").c_str(),DrawIo[14]);
    
    // Removed experimental color toggle
    
    Toggle(i18n::TL("  血量").c_str(),DrawIo[6]);
    ...
    ImGui::PopStyleVar(3);
    ImGui::TextColored(ImColor(255, 255, 255, 255), i18n::TL("关于雷达").c_str());
    SliderFloat (i18n::TL("雷达X").c_str(), &NumIo[1], 10, 3200, "%1.f");
    SliderFloat (i18n::TL("雷达Y").c_str(), &NumIo[2], 10, 1440, "%1.f");
    SliderFloat (i18n::TL("雷达大小").c_str(), &NumIo[19], 10, 300, "%1.f");
    SliderFloat (i18n::TL("雷达缩放").c_str(), &NumIo[16], 70, 150, "%.0f%%", 5);
    SliderFloat (i18n::TL("空余位").c_str(), &NumIo[91], 10, 300, "%1.f");
    ImGui::TextColored(ImColor(150, 150, 150, 255), i18n::TL("数据管理").c_str());
    Toggle("  过滤队伍ID",DrawIo[1000]);
    SliderFloat ("队伍ID", &NumIo[49], 0, 100, "%1.f");
}
void 自瞄_1() {
    TouchScroll(true);
    ...
    ImGui::TextColored(ImColor(255, 255, 255, 255), i18n::TL("关于调节").c_str());
...
void 物资_1() {
    ...
    CenteredText(i18n::TL("物 资").c_str(), 1.5f);
    // Removed loot initialization toggle
    ImGui::TextColored(ImColor(255, 255, 255, 255), i18n::TL("载具类").c_str());
    Toggle("  载具", 物资[1]);
    Toggle("  盒子", 物资[45]);
    Toggle("  宝箱状态", DrawIo[910]);
    ImGui::Spacing();
    ImGui::TextColored(ImColor(255, 255, 255, 255), i18n::TL("武器类").c_str());
    Toggle("  宝箱", 物资[18]);
    Toggle("  信号枪", 物资[3]);
    Toggle("  召回信号枪", 物资[9]);
    ImGui::Spacing();
    ImGui::TextColored(ImColor(255, 255, 255, 255), i18n::TL("药品类").c_str());
    Toggle("  止痛药", 物资[5]);
    Toggle("  饮料", 物资[6]);
    Toggle("  肾上腺素", 物资[7]);
    Toggle("  自救器", 物资[8]);
    Toggle("  药品", 物资[11]);
    ImGui::Spacing();
    ImGui::TextColored(ImColor(255, 255, 255, 255), i18n::TL("特殊类").c_str());
    Toggle("  金插", 物资[4]);
    // Removed BOSS toggle
    Toggle("  超级物资", 物资[60]);
    ImGui::Spacing();
