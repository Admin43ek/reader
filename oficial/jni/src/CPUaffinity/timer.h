...
struct timer {
    ...
    // 使用 AotuFPS 之前需调用 SetFps，参数单位为每秒循环次数
    ...
    long long oldtimer;
    // 调用 AotuFPS 之前请调用 AotuFPS_init 和 SetFps，SetFps 可实时调整FPS。此函数建议放在循环最前面。函数返回值为当前循环时间，单位毫秒，1000.0f / 返回值可得到准确fps
