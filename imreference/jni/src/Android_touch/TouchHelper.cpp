...
// ===============================
//  Эмуляция тач‑событий (uinput)
//
//  Назначение:
//   - Поддержка виртуальных касаний для аимбота и UI (Touch_Down/Move/Up)
//   - Поиск устройства /dev/input/eventX
//   - Нормализация координат под ориентацию экрана
//
//  Осторожно:
//   - Много системных вызовов; вносить изменения аккуратно
//   - Все задержки usleep должны быть минимальны, чтобы не блокировать рендер
// ===============================
...
static bool g_forwardedSlots[10] = {false};
...
static bool ShouldBlockTouchForwarding(int slot)
{
    if (slot >= 5)
        return false;
    ...
    if (ctx == nullptr)
        return false;
    const ImGuiIO& io_ref = ctx->IO;
    return io_ref.WantCaptureMouse || io_ref.WantCaptureMouseUnlessPopupClose;
}

static void EmitSlotRelease(int slot, const timeval& when)
{
    if (dev_fd <= 0)
        return;
    ...
    if (last_slot != slot)
    {
        events[count].type = EV_ABS;
        events[count].code = ABS_MT_SLOT;
        events[count].value = slot;
        events[count].time = when;
        count++;
        last_slot = slot;
    }

    events[count].type = EV_ABS;
    events[count].code = ABS_MT_TRACKING_ID;
    ...
    events[count].value = 0;
    events[count].time = when;
    count++;

    write(dev_fd, events, sizeof(struct input_event) * count);
    touch_status = false;
}
...
void Upload(int slot) {
    ...
    bool fingerUpEvent = (Finger.status == FINGER_UP);

    bool blockTouch = ShouldBlockTouchForwarding(slot);
    grabImGuiEvent = blockTouch;
    if (blockTouch) {
        if (g_forwardedSlots[slot]) {
            EmitSlotRelease(slot, Finger.time);
            g_forwardedSlots[slot] = false;
        }
    ...
    g_forwardedSlots[slot] = !fingerUpEvent;
}
