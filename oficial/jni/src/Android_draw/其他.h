...
void DrawHealth(Vec2 box, float w, float iHealth, uint32_t TeamID, char* name) {
    ...
    auto drawWeaponLabel = [&](ImVec2 min, ImVec2 max, float fontSize, ImU32 textColor = IM_COL32_WHITE) {
        if (!DrawIo[10] || weaponId <= 0) return;
        char weaponLabel[32];
        snprintf(weaponLabel, sizeof(weaponLabel), "ID:%d", weaponId);
        ImVec2 textSize = ImGui::GetFont()->CalcTextSizeA(fontSize, FLT_MAX, -1, weaponLabel);
        ImVec2 labelMin = min - ImVec2(2.0f, 2.0f);
        ImVec2 labelMax = max + ImVec2(2.0f, 2.0f);
        bg->AddRectFilled(labelMin, labelMax, ImColor(0, 0, 0, 105), 6.0f);
        ImVec2 textPos = ImVec2(min.x + (max.x - min.x - textSize.x) * 0.5f,
                                min.y + (max.y - min.y - textSize.y) * 0.5f);
        fg->AddText(ImGui::GetFont(), fontSize, textPos, textColor, weaponLabel);
    };
    ...
    switch (style) {
        case 0: {
            ...
            if (DrawIo[10] && weaponId > 0) {
                ImVec2 iconMin(x - 42.0f, y - 52.0f);
                ImVec2 iconMax(x + 42.0f, y - 4.0f);
                drawWeaponLabel(iconMin, iconMax, 20.0f);
            }
            break;
        }
        case 1: {
            ...
            if (DrawIo[10] && weaponId > 0) {
                ...
                drawWeaponLabel(iconMin, iconMax, 18.0f);
            }
            break;
        }
        case 2: {
            ...
            if (DrawIo[10] && weaponId > 0) {
                ...
                drawWeaponLabel(iconMin, iconMax, 18.0f, IM_COL32_WHITE);
            }
