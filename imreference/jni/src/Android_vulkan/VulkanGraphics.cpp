...
static void check_vk_result(VkResult err) {
    if (err == 0)
        return;
    // suppressed stderr output
    // In debug, don't abort: mark swapchain rebuild or log only.
}

#else

static void check_vk_result(VkResult err) {
    // Release: ignore errors to avoid aborting execution.
}
...
bool VulkanGraphics::Create(ANativeWindow *Window, int m_Width, int m_Height) {
    ...
    if (InitVulkan() != 1) {
        // suppressed stderr output
        // Skip aborting: indicate failure to caller
        return false;
    }
...
 
ImTextureID VulkanGraphics::ImAgeHeadFile(const unsigned char *buf, int len) {
    ...
    if (image_data == nullptr)
        return (ImTextureID)0; // 确保返回正确的空值表示。

    // 假设 LoadTexture 返回一个指向 VulkanTextureData 的指针。
    auto *result = LoadTexture(&tex_data, image_data);
    if (result == nullptr)
        return (ImTextureID)0; // 如果 LoadTexture 失败，返回正确的空值表示。

    stbi_image_free(image_data); // 释放图像数据

    return (ImTextureID)result->DS; // 假设 DS 是一个可以转换为 ImTextureID 的类型。
}
 
